...
class Reflection(obfuscator_category.ICodeObfuscator):
    ...
    def method_is_all_public(self, class_name: str, method_signature: str, param_string: str) -> bool:
        ...
        with open(smali_file, 'r', encoding='utf-8') as current_file:
            for line in current_file:
                if ' public ' in line:
                    ...
                    if method_match:
                        signature = '{method_name}({method_param}){method_return}'.format(
                        ...
                        if signature == method_signature:
                            # Public method declared in public class, let's check if all its parameters are public.
                            for param in self.split_method_params(param_string):
                                # System classes that are public.
                                if param in self.primitive_types or param in self.android_class_names:
                                    continue

                                # The class of this parameter is not present in the smali files or is not public.
                                if not self.class_is_public_and_declared_in_smali(param):
                                    return False
                ...
                if possible_class.startswith('L'):
                    ...
                    for string_position in range(1, len(possible_class)):
                        if possible_class[string_position] == '[':
                            ...
                            # Primitive type array, add it to the list and proceed with the rest of the string
                            params.append(possible_class[:string_position + 1])
                            params.extend(self.split_method_params(possible_class[string_position + 1:]))
                            break
                elif possible_class[0] in self.primitive_types:
                    # Primitive type + other optional parameters (e.g. ILjava/lang/Object).
    ...
    def add_smali_reflection_code(self, class_name: str, method_name: str, param_string: str) -> str:
        ...
        smali_code += '\tconst-class v2, {class_name}\n\n' \
                      '\tconst-string v3, "{method_name}"\n\n'.format(class_name=class_name, method_name=method_name)
        self.obfuscator_instructions_length += 4

        smali_code += '\tinvoke-virtual {v2, v3, v1}, Ljava/lang/Class;->' \
                      'getDeclaredMethod(Ljava/lang/String;[Ljava/lang/Class;)Ljava/lang/reflect/Method;\n\n'
        self.obfuscator_instructions_length += 3

        smali_code += '\tmove-result-object v1\n\n' \
                      '\tsget-object v2, Lcom/apireflectionmanager/ApiReflection;->' \
                      'obfuscatedMethods:Ljava/util/List;\n\n'
        self.obfuscator_instructions_length += 3

        smali_code += '\tinvoke-interface {v2, v1}, Ljava/util/List;->add(Ljava/lang/Object;)Z\n'
        self.obfuscator_instructions_length += 3

        return smali_code

    def create_reflection_method(self, num_of_methods: int, local_count: int, is_virtual_method: bool,
                                 invoke_registers: str, invoke_parameters: str):
        # Split method passed registers (if the method has no registers there is an empty line that has to be
        # removed, that's why strip() is used).
        invoke_registers = [register.strip() for register in invoke_registers.split(', ') if register.strip()]

        params = self.split_method_params(invoke_parameters)

        param_to_register: List[List[str]] = []  # list[i][0] = i-th param, list[i][1] = [i-th param register(s)]

        if is_virtual_method:
            # If this is a virtual method, the first register is the object instance and not a parameter.
            register_index = 1
            for param in params:
                # Long and double variables need 2 registers.
                if param == 'J' or param == 'D':
                    param_to_register.append([param, invoke_registers[register_index:register_index + 2]])
                    register_index += 2
                else:
                    param_to_register.append([param, [invoke_registers[register_index]]])
                    register_index += 1
        else:
            # This is a static method, so we don't need a reference to the object instance.
            # If this is a virtual method, the first register is the object instance and not a parameter.
            register_index = 0
            for param in params:
                # Long and double variables need 2 registers.
        ...
        if len(params) > 0:
            ...
            for param_index, param_and_register in enumerate(param_to_register):
                ...
                cast_primitive_to_class = self.cast_dict.get(param_and_register[0], None)

                if cast_primitive_to_class:
                    if len(param_and_register[1]) > 1:
                        # 2 register parameter.
                        smali_code += '\tinvoke-static {{{register_pair}}}, {cast}\n\n' \
                                      '\tmove-result-object #reg2#\n\n' \
                            .format(register_pair=', '.join(param_and_register[1]), cast=cast_primitive_to_class)
                    else:
                        smali_code += '\tinvoke-static {{{register}}}, {cast}\n\n' \
                                      '\tmove-result-object #reg2#\n\n'.format(register=param_and_register[1][0],
                                                                               cast=cast_primitive_to_class)

                    smali_code += '\tconst/4 #reg4#, {param_index:#x}\n\n' \
                                  '\taput-object #reg2#, #reg1#, #reg4#\n\n'.format(param_index=param_index)

                else:
                    smali_code += '\tconst/4 #reg3#, {param_index:#x}\n\n' \
                                  '\taput-object {register}, #reg1#, #reg3#\n\n' \
                        .format(param_index=param_index, register=param_and_register[1][0])

        smali_code += '\tconst/16 #reg3#, {method_num:#x}\n\n'.format(method_num=num_of_methods)

        if is_virtual_method:
            smali_code += '\tinvoke-static {{#reg3#, {obj_instance}, #reg1#}}, ' \
            ...
            smali_code += '\tinvoke-static {#reg3#, #reg4#, #reg1#}, ' \
                          'Lcom/apireflectionmanager/ApiReflection;->' \
                          'obfuscate(ILjava/lang/Object;[Ljava/lang/Object;)Ljava/lang/Object;\n'

        for index in range(0, 4):
            smali_code = smali_code.replace('#reg{0}#'.format(index + 1), 'v{0}'.format(local_count + index))
    ...
    def obfuscate(self, obfuscation_info: Obfuscation):
        ...
        try:
            ...
            for smali_file in util.show_list_progress(obfuscation_info.get_smali_files(),
                ...
                self.logger.debug('Obfuscating using reflection in file "{0}"'.format(smali_file))
                ...
                # Look for method invocations inside the methods declared in this smali file and change normal
                # invocations with invocations through reflection.
                    ...
                    if method_is_reflectable[method_number]:
                        ...
                        while not lines[current_line_number].startswith('.end method'):
                            ...
                            if invoke_match and '<init>' not in lines[current_line_number]:
                                ...
                                # The method belongs to an Android class or is invoked on an array.
                                if invoke_match.group('invoke_object') in self.android_class_names or \
                                        invoke_match.group('invoke_object').startswith('['):
                                    continue

                                method_signature = '{method_name}({method_param}){method_return}'.format(
                                ...
                                # The method to reflect has to be public, has to be declared in a public class and all
                                # its parameters have to be public.
                                if not self.method_is_all_public(invoke_match.group('invoke_object'), method_signature,
                                ...
                                for move_result_index in range(current_line_number + 1,
                                ...
                                obfuscator_smali_code += self.add_smali_reflection_code(tmp_class_name, tmp_method,
                                                                                        tmp_param)

                                # Change the original code with code using reflection.
                                lines[current_line_number] = self.create_reflection_method(
                                    self.methods_with_reflection, method_local_count[method_number],
                                    tmp_is_virtual, tmp_register, tmp_param)

                                self.methods_with_reflection += 1

                                # Add the registers needed for performing reflection.
                                lines[index + 1] = '\t.locals {0}\n'.format(method_local_count[method_number] + 4)
