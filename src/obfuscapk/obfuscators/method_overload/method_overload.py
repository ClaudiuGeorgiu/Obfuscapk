...
class MethodOverload(obfuscator_category.ICodeObfuscator):
            ...
            with open(smali_file, "r", encoding="utf-8") as current_file:
                ...
                for line in current_file:
                    ...
                    if not class_name:
                        ...
                        # belongs to the classes to ignore. If this is a class to
                        # ignore, get its methods and add them to the list of methods
                        # to be ignored when performing method overloading.
                        class_match = util.class_pattern.match(line)
                        if class_match:
                            class_name = class_match.group("class_name")
                            if class_name not in class_names_to_ignore:
                                # The methods of this class shouldn't be ignored when
                                # performing method overloading, so proceed with the
                                # next class.
                                break
                            else:
                                continue
                    ...
                    # avoided also when performing method overloading).
                    if (
                        method_match
    ...
    def add_method_overloads_to_file(
        self, smali_file: str, overloaded_method_body: str, methods_to_ignore: Set[str]
    ) -> int:
